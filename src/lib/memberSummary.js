...
function summarize(goals, events, consistency, timeZone, now = new Date()) {
  ...
  const awards=categories.map(category=>{
    ...
    return {category,taskAwards:eligible.map(g=>({id:g.tag,text:`#${g.tag}`,targetDays:g.subtasks.reduce((n,t)=>n+t.targetDays,0),completedDays:g.subtasks.reduce((n,t)=>n+t.completedDays,0)})),completedGoals:completed,goalPercent:nearest?.percent||0,nearestGoal:nearest?.tag||null,bestStreak:earnedStreak?.best_streak||0,consistencyEarned:!!earnedStreak?.earned_at};
  });
