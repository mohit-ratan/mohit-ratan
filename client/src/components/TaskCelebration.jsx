...
export default function TaskCelebration({ celebration, onClose }) {
  const { day, targetDays, taskName, tag, streakDays, goalCompleted, taskCompleted, trifectaEarned } = celebration;
  ...
  const title = trifectaEarned ? 'The Trifecta Award is yours!' : goalCompleted ? 'Goal complete. Award earned!' : taskCompleted ? 'Task complete. You did it!' : titles[level - 1];
  ...
  return <div className={`task-party-backdrop task-party-level-${level}`} onMouseDown={e => { if (e.target === e.currentTarget) onClose(); }}>
    ...
    <section ref={ref} tabIndex={-1} className="task-party-card" role="dialog" aria-modal="true" aria-labelledby="task-party-title" aria-describedby="task-party-summary">
      ...
      <span className="task-party-eyebrow">{trifectaEarned ? 'RARE AWARD UNLOCKED' : goalCompleted ? 'ACHIEVEMENT UNLOCKED' : taskCompleted ? 'TASK MILESTONE' : 'PHOTO SAVED · PROGRESS EARNED'}</span>
      {trifectaEarned && (
