...
function fixture({ done = false, missing = false, fail = false, targetDays = 1 } = {}) {
  ...
  const sandbox = { module: { exports: {} }, console: { error() {} }, require: (name) => name === '../db' ? { getConnection: async () => connection } : name === 'uuid' ? { v4: () => 'photo-id' } : name === '../lib/goalProgress' ? require('../src/lib/goalProgress') : name === '../lib/follows' ? { canView: async () => true } : name === '../lib/notifications' ? { notify: async () => {} } : name === '../lib/storage' ? { uploadMedia: async () => '/assets/uploads/small.jpg', deleteMedia: async () => {} } : require(name) };
